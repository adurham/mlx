...
void ResidencySet::insert(MTL::Allocation* buf) {
  ...
  // certain (undocumented) conditions — confirmed via DYLD interposer on
  // DSv4-Flash long-decode SIGABRTs:
  //   abort()
  //   IOGPUMetalResidencySet removeAllocation:
  //   ResidencySet::insert            ← here
  //   MetalAllocator::malloc
  //   ArgPartition::eval_gpu
  //   async_eval                      ← every decode step
  // Adding even with a 5% capacity-margin guard didn't help — Apple's
  // abort path fires regardless of size headroom. Always route through
  // unwired_set_ instead. This loses the GPU-residency hint optimization
  // but never aborts the process.
