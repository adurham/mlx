...
ResidencySets::ResidencySets(MTL::Device* d) {
  ...
  // FORK-LOCAL (2026-06, f8bac6422 / 858dfd3b6, carried across upstream's
  // 2026-09 multi-set rewrite): keep the residency sets DISABLED by default.
  //
  // Apple's IOGPUMetalResidencySet calls abort() from inside
  // addAllocation/removeAllocation under undocumented conditions -- confirmed
  // via a DYLD interposer on DSv4-Flash long-decode SIGABRTs:
  //     abort()
  //     IOGPUMetalResidencySet removeAllocation:
  //     ResidencySet::insert
  //     MetalAllocator::malloc
  //     ArgPartition::eval_gpu
  //     async_eval                      <- every decode step
  // A capacity-margin guard did not help; the abort fired regardless of size
  // headroom. exo DOES raise the wired limit at model load
  // (utils_mlx.py: set_wired_limit_for_model), so upstream's rewritten
  // insert()/resize() would reach addAllocation in production and can still
  // abort the runner. Losing the GPU-residency hint costs some performance but
  // never kills the process.
  //
  // Set MLX_RESIDENCY_SETS=1 to opt back into upstream's behaviour (e.g. to
  // re-measure on a newer macOS where the abort may be fixed).
  static const bool residency_sets_enabled = [] {
    const char* v = std::getenv("MLX_RESIDENCY_SETS");
    return v != nullptr && v[0] == '1';
  }();
  if (!residency_sets_enabled) {
    return; // enabled_ stays false -> every method below is a no-op
  }
