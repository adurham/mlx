...
[[kernel]] void fp_gather_qmv_rhs(
    ...
    const constant int& num_rows [[buffer(7)]],
    uint3 tid [[threadgroup_position_in_grid]],
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  constexpr int packs_per_thread = 2;
  ...
  const device uint8_t* ws = (const device uint8_t*)w +
      ...
      out_row * in_vec_size_w + simd_lid * packs_per_thread * bytes_per_pack;
  const device uint8_t* sb = scales +
      size_t(expert) * out_vec_size * in_vec_size_g +
...
// gather_qmm_rhs with lhs_indices for the fp (mxfp4/nvfp4) modes — no
// broadcast. Port of affine_gather_qmm_rhs_lhs (OPT-9, 2026-06-24) with the
// fp quantization delta: uint8_t scales, no bias tensor, and the fp
// pack-factor helpers. Same run-length encoding as fp_gather_qmm_rhs, reads x
