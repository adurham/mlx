...
void EventImpl::wait(uint64_t value) {
  ...
  // Upstream 2026-09 moved error propagation onto Event/Error (an
  // std::atomic<Error*> poisoned by a failed command buffer). EventImpl no
  // longer has its own check_error(), so read the pointer directly -- same
  // semantics: Error::check() throws and clears.
  auto check_error = [this] {
    if (auto* p = error_.load(); p != nullptr) {
      p->check();
    }
  };
  check_error();
