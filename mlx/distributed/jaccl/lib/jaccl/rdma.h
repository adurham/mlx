...
class SideChannel {
 public:
  ...
  // across ranks.
  //
  // NOTE (2026-07-17, revision 2): called on a DEDICATED SideChannel
  // (p2p_channel_ in mesh.h/mesh_impl.h), NOT the shared coordinator_ used
  // by reliable_barrier()/reliable_all_reduce. The first implementation of
  // this retry protocol called p2p_retry_barrier on the SHARED coordinator_
  // socket, which ALSO carries mlx-lm's model-level mx.distributed.all_gather
  // traffic (deepseek_v4.py, runs every forward pass) via reliable_all_reduce
  // -- those two independent, unsynchronized users interleaved bytes on one
  // socket, corrupting both streams' framing (confirmed empirically: a
  // peer's frame showed this method's own MAGIC bytes landed in the wrong
  // field). This method itself is unchanged; the fix was giving send()/
  // recv() their own dedicated SideChannel instance to call it on.
