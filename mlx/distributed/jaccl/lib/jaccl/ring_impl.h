...
class RingImpl {
 public:
  ...
  void send_wire(
    ...
    // PP p2p send fix (fork-local): force small buffer class
    // (sz=0, N=FRAME_SIZE=4KB). Large UC sends silently drop (receiver RQ
    // overrun, no retransmit on UC). 64KB (sz=4) still drops. 4KB works but
    // is slow (4096 chunks for 16MB). The real fix is routing p2p through
    // the reliable ARQ path.
  ...
  void recv_wire(
    ...
    // PP p2p recv fix (fork-local): match the send-side 4KB cap.
    auto [sz, N] = std::pair<int, int64_t>{0, FRAME_SIZE};
    (void)bytes_per_wire;
