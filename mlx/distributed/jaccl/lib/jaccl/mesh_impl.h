...
class MeshImpl {
 public:
  ...
  // Reliable point-to-point send with drop-recovery via bounded retry over
  // the TCP coordinator (2026-07-17). ROOT-CAUSE: raw send()/recv() used to
  // be bare UC with only a fatal 8s stall watchdog -- a single dropped
  // packet (no NAK, no completion on either side; UC silently vanishes it)
  // meant permanent stall -> runner crash -> full re-place. Confirmed live
  // during a 300K-context decode: 36 clean round-trips, then one drop at
  // call_id=610 killed the whole request. TP's collectives already have
  // this exact problem solved (reliable_all_reduce, see its doc comment);
  // this ports that pattern -- chunk header + receiver got-bitmask + TCP-
  // barrier retransmit-missing loop -- adapted for ONE-DIRECTIONAL transfer
  // (no reduce; only the receiver tracks a got bitmask, the sender always
  // "has everything" since it's the source).
  //
  // IMPORTANT: retransmits are reconstructed from the ORIGINAL in_ptr at
  // the chunk's byte offset, NOT from a retained send_buffer slot. A UC
  // send-side completion only confirms wire transmission, not delivery --
  // by the time a drop is discovered (a full round later), the pipeline
  // may have already overwritten that buffer slot with a later chunk's
  // data. Recomputing from source is correct and needs no slot-retention
  // bookkeeping (verified with Fable before implementing).
  void send(uint32_t call_id, const char* in_ptr, int64_t n_bytes, int dst) {
    ...
    // reliable TCP coordinator for its retransmit-negotiation barrier --
    ...
    // has a coordinator_ (side_channel_ is populated in that ctor; only
    ...
    if (coordinator_ == nullptr) {
      throw std::runtime_error(
          "[jaccl] send() called with no reliable TCP coordinator_ -- the "
    ...
    for (int round = 0;; round++) {
      ...
      // framed p2p_retry_barrier (NOT the raw coordinator_->all_gather)
      ...
      auto peer_got = coordinator_->p2p_retry_barrier(
  ...
  void recv(uint32_t call_id, char* out_ptr, int64_t n_bytes, int src) {
    ...
    // requires the reliable TCP coordinator.
    if (coordinator_ == nullptr) {
      throw std::runtime_error(
          "[jaccl] recv() called with no reliable TCP coordinator_ -- the "
          "retry-on-drop protocol requires it; only top-level groups have "
          "one, and send()/recv() should only be called on a top-level "
          "group (exo's Pipeline-Parallel usage)");
    }
    ...
    for (int round = 0;; round++) {
      ...
      // fixed-tag) is used instead of raw coordinator_->all_gather.
      coordinator_->p2p_retry_barrier(
