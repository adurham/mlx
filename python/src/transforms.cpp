...
void init_transforms(nb::module_& m) {
  ...
  m.def(
      ...
      []() {
        ...
        //
        // upstream-sync-2026-09: compile_clear_cache() gained a
        // CompileCacheWeakPtr parameter in ml-explore/mlx@8e00a2d9d.
        // detail::compile_cache() returns the *calling thread's* cache, which
        // is exactly the object this fork-only binding cleared before, so
        // passing it here preserves the original per-thread semantics.
        mx::detail::compile_clear_cache(mx::detail::compile_cache());
      },
      R"pbdoc(
